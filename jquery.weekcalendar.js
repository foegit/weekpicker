...
//              calendarHeaderHtml+=    "</div>";
  ...
  TODO    if total height for an hour is less than 11px, there is a display problem.
      ...
      _updateDayColumnHeader: function($weekDayColumns) {
          ...
          var activeClass = 'ui-state-active activeRow';
      ...
      _addDraggableToCalEvent: function(calEvent, $calEvent) {
        ...
        $calEvent.draggable({
          ...
          start: function(event, ui) {
            ...
            function getFormatedNumber(index) {
              return (index > 9 ? '':'0') + index;
            };

            function getAmOrPm(index) {
              return self._amOrPm(index).toLocaleLowerCase();
            };

            function getFormatedTime(date) {
              const hour = getFormatedNumber(self._hourForIndex(date.getHours()));
              const minute = getFormatedNumber(date.getMinutes());
              const partOfDay = getAmOrPm(date.getHours());

              return `${hour}:${minute} ${partOfDay}`;
            }

            function getFormatedRange(startTime, endTime) {
              const start = new Date(startTime);
              const end = new Date(endTime);
              
              return `${getFormatedTime(start)} to ${getFormatedTime(end)}`;
            }

            $($calEvent).bind('mousemove', (event) => {
              const top = Math.round(parseInt($(event.target).parent().position().top));
              const eventDuration = self._getEventDurationFromPositionedEventElement($weekDay, $calEvent, top);
              let el = $(event.target).parent();

              el.find('.wc-time').text(getFormatedRange(eventDuration.start, eventDuration.end));
            });
          }
        });
      },
